    ...
    <div style={{ backgroundColor: "#F5F2EA", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "28px 20px", fontFamily: "Inter, sans-serif" }}>
      <div style={{ width: "100%", maxWidth: 540 }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          <h2 style={{ fontSize: 22, fontWeight: 700, color: "#1F3D2C", margin: "0 0 8px" }}>Grant us access to your accounts</h2>
          <p style={{ fontSize: 13, color: "#6B7280", margin: "0 0 14px", lineHeight: 1.5 }}>
            Follow the steps for each platform you use. You stay in control — you can remove access at any time.
          </p>
          {/* Progress */}
          <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "white", border: "1px solid rgba(31,61,44,0.12)", borderRadius: 20, padding: "5px 14px" }}>
            <div style={{ display: "flex", gap: 4 }}>
              {PLATFORMS.map(p => (
                <div key={p.id} style={{ width: 8, height: 8, borderRadius: "50%", background: completed[p.id] ? "#2D6B4F" : "rgba(31,61,44,0.12)", transition: "background 0.2s" }}/>
              ))}
            </div>
            <span style={{ fontSize: 12, fontWeight: 600, color: "#1F3D2C" }}>{completedCount} of {PLATFORMS.length} done</span>
          </div>
        </div>
