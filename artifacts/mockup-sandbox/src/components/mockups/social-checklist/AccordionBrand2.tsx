...
export function AccordionBrand2() {
  ...
  return (
    <div style={{ background: "#F5F2EA", minHeight: "100vh", fontFamily: "Inter, sans-serif" }}>
      ...
      {/* Bold forest header with rust bottom bar */}
      <div style={{ background: "#1F3D2C", borderBottom: "5px solid #7D2A03" }}>
        <div style={{ maxWidth: 520, margin: "0 auto", padding: "30px 28px 26px" }}>
          <p style={{ fontSize: 10, fontWeight: 800, letterSpacing: "0.14em", textTransform: "uppercase", color: "#7D2A03", margin: "0 0 12px" }}>
            Social Media Access
          </p>
          <h2 style={{ fontSize: 30, fontWeight: 900, color: "white", margin: "0 0 10px", lineHeight: 1.1, letterSpacing: "-0.025em" }}>
            Grant us access
          </h2>
          <p style={{ fontSize: 13, color: "rgba(235,201,155,0.75)", margin: "0 0 22px", lineHeight: 1.65 }}>
            To get started managing your social media, we'll need admin access to your accounts. Follow the steps below — reach out if you get stuck.
          </p>
          <div style={{ display: "inline-flex", background: "rgba(255,255,255,0.08)", borderRadius: 6, padding: 3, gap: 2 }}>
            {PLANS.map(p => (
              <button key={p.id} onClick={() => { setPlan(p.id as "starter" | "growth"); setSelected([]); setCompleted({}); setUrls({}); setOpenId(null); }}
                style={{ padding: "6px 16px", borderRadius: 4, border: "none", cursor: "pointer", fontSize: 11, fontWeight: 700, fontFamily: "inherit", background: plan === p.id ? "#7D2A03" : "transparent", color: plan === p.id ? "white" : "rgba(255,255,255,0.5)", transition: "all 0.18s" }}>
                {p.label}
              </button>
            ))}
          </div>
      ...
      <div style={{ maxWidth: 520, margin: "0 auto", padding: "24px 20px 40px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <div style={{ width: 4, height: 16, borderRadius: 2, background: "#7D2A03" }}/>
            <span style={{ fontSize: 12, fontWeight: 800, color: "#1F3D2C", textTransform: "uppercase", letterSpacing: "0.06em" }}>Your platforms</span>
          </div>
          ...
          {PLATFORMS.map(p => {
            ...
            return (
              <button key={p.id} onClick={() => toggleSelect(p.id)} disabled={isLocked}
                style={{ position: "relative", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8, padding: "18px 12px", borderRadius: 10, background: isDone ? "rgba(45,107,79,0.08)" : isSel ? "#2D6B4F" : "#F5F2EA", border: `2px solid ${isDone ? "#2D6B4F" : isSel ? "#2D6B4F" : "rgba(31,61,44,0.12)"}`, cursor: isLocked ? "not-allowed" : "pointer", opacity: isLocked ? 0.4 : 1, fontFamily: "inherit", transition: "all 0.15s", boxShadow: isSel ? "0 4px 14px rgba(45,107,79,0.25)" : "none" }}>
                {isSel && !isDone && (
                  <div style={{ position: "absolute", top: 8, right: 8, width: 18, height: 18, borderRadius: "50%", background: "rgba(255,255,255,0.25)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20,6 9,17 4,12"/></svg>
                  </div>
                )}
                <span style={{ color: isSel ? "white" : "#1F3D2C" }}>{p.icon}</span>
                <span style={{ fontSize: 12, fontWeight: 700, color: isSel ? "white" : "#1F3D2C" }}>{p.name}</span>
        ...
        {atLimit && <p style={{ fontSize: 11, color: "#7D2A03", background: "rgba(125,42,3,0.06)", border: "1px solid rgba(125,42,3,0.12)", borderRadius: 6, padding: "8px 12px", marginBottom: 16 }}>Plan limit reached. Tap a platform to swap.</p>}
        ...
        {selected.length > 0 && (
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 18, padding: "10px 16px", background: "white", borderRadius: 10, border: "1px solid rgba(31,61,44,0.08)" }}>
            ...
            <span style={{ fontSize: 12, color: "#6B7280" }}>
              {completedCount === 0 ? "Tap a platform below to get started" : completedCount === selected.length ? <span style={{ color: "#7D2A03", fontWeight: 700 }}>All done!</span> : `${selected.length - completedCount} left`}
            </span>
          </div>
        )}
        ...
        {selected.length === 0 ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {PLATFORMS.filter(p => selected.includes(p.id)).map(p => {
              ...
              const isDone = completed[p.id];
              return (
                <div key={p.id} style={{ background: "white", borderRadius: 10, overflow: "hidden", border: `2px solid ${isOpen ? "#1F3D2C" : isDone ? "rgba(45,107,79,0.25)" : "rgba(31,61,44,0.1)"}`, boxShadow: isOpen ? "0 6px 24px rgba(31,61,44,0.12)" : "0 1px 4px rgba(0,0,0,0.04)", transition: "all 0.2s" }}>
                  {/* Rust top accent when open */}
                  {isOpen && <div style={{ height: 3, background: "#7D2A03" }}/>}

                  <button onClick={() => setOpenId(isOpen ? null : p.id)}
                    style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px", background: isDone ? "rgba(45,107,79,0.05)" : "none", border: "none", cursor: "pointer", textAlign: "left", fontFamily: "inherit" }}>
                  ...
                  {isOpen && (
                    <div style={{ borderTop: "1px solid rgba(31,61,44,0.08)", padding: "16px 16px 18px" }}>
                      <ol style={{ margin: "0 0 14px", padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: 12 }}>
                        {p.steps.map((step, i) => (
                          <li key={i} style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                            <span style={{ flexShrink: 0, width: 24, height: 24, borderRadius: "50%", background: "#7D2A03", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 800, color: "white", marginTop: 1 }}>{i + 1}</span>
