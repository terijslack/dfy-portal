...
const SEL_BG = "rgba(45,107,79,0.09)";
...
export function AccordionBrand1() {
  ...
  return (
    <div style={{ background: "#F5F2EA", minHeight: "100vh", fontFamily: "Inter, sans-serif" }}>
      ...
      {/* Tan editorial header */}
      <div style={{ background: "#EBC99B", padding: "32px 28px 28px" }}>
        <div style={{ maxWidth: 520, margin: "0 auto" }}>
          <p style={{ fontSize: 10, fontWeight: 800, letterSpacing: "0.12em", textTransform: "uppercase", color: "#7D2A03", margin: "0 0 10px" }}>
            Social Media Access
          </p>
          <h2 style={{ fontSize: 28, fontWeight: 900, color: "#1F3D2C", margin: "0 0 10px", lineHeight: 1.15, letterSpacing: "-0.02em" }}>
            Grant us access
          </h2>
          <p style={{ fontSize: 13, color: "rgba(31,61,44,0.75)", margin: "0 0 20px", lineHeight: 1.65, maxWidth: 420 }}>
            To get started managing your social media, we'll need admin access to your accounts. Follow the steps below for each platform you use.
          </p>
          <div style={{ display: "inline-flex", background: "rgba(31,61,44,0.1)", borderRadius: 24, padding: 3, gap: 2 }}>
            {PLANS.map(p => (
              <button key={p.id} onClick={() => { setPlan(p.id as "starter" | "growth"); setSelected([]); setCompleted({}); setUrls({}); setOpenId(null); }}
                style={{ padding: "6px 16px", borderRadius: 20, border: "none", cursor: "pointer", fontSize: 11, fontWeight: 700, fontFamily: "inherit", background: plan === p.id ? "#1F3D2C" : "transparent", color: plan === p.id ? "#EBC99B" : "rgba(31,61,44,0.6)", transition: "all 0.18s" }}>
                {p.label}
              </button>
            ))}
          </div>
      ...
      <div style={{ maxWidth: 520, margin: "0 auto", padding: "24px 20px 40px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
          <span style={{ fontSize: 14, fontWeight: 800, color: "#1F3D2C", letterSpacing: "-0.01em" }}>Pick your platforms</span>
          ...
          {PLATFORMS.map(p => {
            ...
            return (
              <button key={p.id} onClick={() => toggleSelect(p.id)} disabled={isLocked}
                style={{ position: "relative", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8, padding: "18px 12px", borderRadius: 14, background: isSel || isDone ? SEL_BG : "white", border: `2px solid ${isSel || isDone ? SEL_BORDER : "rgba(31,61,44,0.1)"}`, cursor: isLocked ? "not-allowed" : "pointer", opacity: isLocked ? 0.4 : 1, fontFamily: "inherit", transition: "all 0.15s", boxShadow: isSel ? "0 4px 14px rgba(45,107,79,0.12)" : "0 1px 3px rgba(0,0,0,0.04)" }}>
                {isSel && (
                ...
                <span style={{ color: isSel ? "#2D6B4F" : "#1F3D2C" }}>{p.icon}</span>
        ...
        {atLimit && <p style={{ fontSize: 11, color: "#7D2A03", background: "rgba(125,42,3,0.06)", border: "1px solid rgba(125,42,3,0.12)", borderRadius: 8, padding: "8px 12px", marginBottom: 16 }}>Plan limit reached. Tap a selected platform to swap.</p>}

        {/* Progress bar */}
        {selected.length > 0 && (
          <div style={{ marginBottom: 18 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
              <span style={{ fontSize: 11, fontWeight: 700, color: "#1F3D2C", textTransform: "uppercase", letterSpacing: "0.06em" }}>Progress</span>
              <span style={{ fontSize: 11, fontWeight: 700, color: completedCount === selected.length ? "#7D2A03" : "#2D6B4F" }}>
                {completedCount === selected.length ? "All done!" : `${completedCount}/${selected.length} complete`}
              </span>
            </div>
            <div style={{ height: 6, background: "#EBC99B", borderRadius: 6, overflow: "hidden" }}>
              <div style={{ height: "100%", width: `${progress}%`, background: "linear-gradient(90deg, #2D6B4F, #1F3D2C)", borderRadius: 6, transition: "width 0.4s ease" }}/>
            </div>
          </div>
        )}
        ...
        {selected.length === 0 ? (
          <div style={{ textAlign: "center", padding: "32px 20px", background: "white", borderRadius: 14, border: "2px dashed rgba(31,61,44,0.12)" }}>
            <p style={{ fontSize: 13, color: "#9CA3AF", margin: 0 }}>Select platforms above to get started.</p>
          </div>
        ) : (
              ...
              return (
                <div key={p.id} style={{ background: "white", borderRadius: 14, overflow: "hidden", border: `2px solid ${isOpen ? "#2D6B4F" : isDone ? "rgba(45,107,79,0.25)" : "rgba(31,61,44,0.1)"}`, boxShadow: isOpen ? "0 6px 24px rgba(31,61,44,0.1)" : "0 1px 4px rgba(0,0,0,0.04)", transition: "all 0.2s" }}>
                  <button onClick={() => setOpenId(isOpen ? null : p.id)}
                    style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px", background: isDone ? "rgba(235,201,155,0.2)" : "none", border: "none", cursor: "pointer", textAlign: "left", fontFamily: "inherit" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      <div style={{ width: 42, height: 42, borderRadius: 12, background: isDone ? "rgba(45,107,79,0.08)" : "#EBC99B22", border: "1.5px solid rgba(235,201,155,0.6)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                        {isDone
                    ...
                    <div style={{ width: 28, height: 28, borderRadius: "50%", background: isOpen ? "#1F3D2C" : "#F5F2EA", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "background 0.2s" }}>
                      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke={isOpen ? "#EBC99B" : "#9CA3AF"} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ transform: isOpen ? "rotate(180deg)" : "rotate(0)", transition: "transform 0.25s" }}>
                  ...
                  {isOpen && (
                    <div style={{ borderTop: "2px solid rgba(235,201,155,0.4)", padding: "16px 16px 18px" }}>
                      <ol style={{ margin: "0 0 14px", padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: 12 }}>
                        {p.steps.map((step, i) => (
                          <li key={i} style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                            <span style={{ flexShrink: 0, width: 24, height: 24, borderRadius: "50%", background: "#EBC99B", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 800, color: "#1F3D2C", marginTop: 1 }}>{i + 1}</span>
                            <span style={{ fontSize: 13, color: "#374151", lineHeight: 1.55, paddingTop: 3 }}>{step}</span>
                          </li>
                        ))}
                      </ol>
                      <div style={{ background: "rgba(235,201,155,0.2)", border: "1px solid rgba(235,201,155,0.6)", borderRadius: 10, padding: "10px 14px", marginBottom: 14, display: "flex", gap: 8 }}>
                        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#1F3D2C" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0, marginTop: 1 }}><polyline points="20,6 9,17 4,12"/></svg>
