/* Variant C — Logo inside the Hero Testimonial Card
   Everything is identical to the real homepage.
   The only addition: a small branded logo icon sits at the top of the
   dark green testimonial card, turning it into a clearly DFY-branded element. */
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif", background: "#F5F2EA", color: "#6B756B" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @keyframes pulse-ring {
          0%   { box-shadow: 0 0 0 0 rgba(235,201,155,0.80); }
          70%  { box-shadow: 0 0 0 12px rgba(235,201,155,0); }
          100% { box-shadow: 0 0 0 0 rgba(235,201,155,0); }
        }
        .logo-highlight { animation: pulse-ring 2s ease-in-out infinite; border-radius: 10px; display: inline-flex; }
      `}</style>
      ...
      <section style={{ padding: "96px 48px 80px", maxWidth: 1100, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "center" }}>
        ...
        {/* Right col — testimonial card with logo added at top */}
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ background: "#1F3D2C", borderRadius: 20, padding: "40px 36px", boxShadow: "0 8px 40px rgba(31,61,44,0.20)", position: "relative", overflow: "hidden" }}>
            ...
            {/* ★ LOGO PLACEMENT: branded header of the card */}
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 24, position: "relative" }}>
              ...
              <div>
                <div style={{ fontFamily: "'Fraunces',Georgia,serif", fontSize: 13, fontWeight: 600, color: "#F5F2EA" }}>Done For You Marketing</div>
                <div style={{ fontSize: 10, fontWeight: 600, color: "#EBC99B", letterSpacing: "0.12em", textTransform: "uppercase" }}>Client Story</div>
              </div>
              {/* Annotation */}
              <div style={{ position: "absolute", right: 0, top: -4, background: "#EBC99B", color: "#1F3D2C", fontSize: 10, fontWeight: 700, padding: "3px 8px", borderRadius: 4, whiteSpace: "nowrap", letterSpacing: "0.06em" }}>
                ★ LOGO HERE
              </div>
            </div>
...
function LogoMark({ size = 40, light = false }: { size?: number; light?: boolean }) {
  return (
    <svg width={size} height={size} viewBox="0 0 80 80" fill="none">
      <path d="M40 4L73 22V58L40 76L7 58V22L40 4Z" stroke="#EBC99B" strokeWidth="3.5" fill="rgba(235,201,155,0.12)" />
      <line x1="40" y1="52" x2="40" y2="30" stroke={light ? "#6EC89A" : "#2D6B4F"} strokeWidth="2.8" strokeLinecap="round" />
      <path d="M40 38 C40 38 32 34 30 26 C36 26 40 32 40 38Z" fill={light ? "#6EC89A" : "#2D6B4F"} />
      <path d="M40 44 C40 44 48 40 50 32 C44 32 40 38 40 44Z" fill={light ? "#6EC89A" : "#2D6B4F"} />
      <ellipse cx="40" cy="53" rx="6" ry="2.5" fill={light ? "rgba(110,200,154,0.30)" : "rgba(45,107,79,0.25)"} />
    </svg>
  );
}
